...
import {AggregatorV3Interface} from "lib/chainlink-brownie-contracts/contracts/src/v0.8/interfaces/AggregatorV3Interface.sol";
 ...
 contract DSCEngine is ReentrancyGuard {
    ...
    mapping(address user => uint256 amountDskMinted) private s_DSCMinted;
    ...
    function liquidate(address collateral, address user, uint256 debtToCov)
        external
        moreThanZero(debtToCov)
        nonReentrant
    {
        uint256 startingUserHelthFactor = _healthFactor(user);
        if (startingUserHelthFactor >= MIN_HEALTH_FACTOR) {
    ...
    function _healthFactor(address user) private view returns (uint256) {
        ...
        uint256 collateralAdjustedForThreshold = (collateralValueInUsd * LIQUIDATION_PRECISION) / LIQUIDATION_PRECISION;
